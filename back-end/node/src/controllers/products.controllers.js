...
export const getProducts = async (req, res) => /*res.send ('obteniendo clientes')*/ {
    try {
        const [rows] = await pool.query('SELECT id_product, title,descrip,name_brand,name_category,name_color,quantify,price,url FROM products INNER JOIN  brand ON products.brand_product = brand.id_brand INNER JOIN category ON products.category = category.id_category INNER JOIN colors ON products.color = colors.id_color')
        res.json(rows)
    } catch (error) {
        console.log(error)
